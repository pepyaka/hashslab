use std::vec;
use std::{collections::HashSet, println, string::String, vec::Vec};
...
#[test]
fn new() {
    let set = HashSlabSet::<String>::new();
    println!("{:?}", set);
...
fn insert() {
    ...
    let mut set = HashSlabSet::with_capacity(insert.len());
...
#[test]
fn insert_2() {
    let mut set = HashSlabSet::with_capacity(16);
...
fn insert_dup() {
    ...
    let mut set: HashSlabSet<u8> = elements.drain(..).collect();
    {
...
fn replace() {
    ...
    let not_present = [1, 3, 6, 9, 10];
    let mut set = HashSlabSet::with_capacity(replace.len());
...
fn replace_order() {
    ...
    for &elt in &replace {
        assert!(set.replace(elt).is_none());
    }

    for &elt in &replace {
        assert!(set.replace(elt).is_some());
    }
...
#[test]
fn reserve() {
    let mut set = HashSlabSet::<usize>::new();
    assert_eq!(set.capacity(), 0);
    set.reserve(100);
...
fn remove() {
    ...
    for &value in &remove_fail {
        assert!(set.remove_full(&value).is_none());
    }
    println!("{:?}", set);
    for &value in &remove {
        //println!("{:?}", set);
        let index = set.get_full(&value).unwrap().0;
        assert_eq!(set.remove_full(&value), Some((index, value)));
    }
    println!("{:?}", set);
...
fn remove_index() {
    ...
    let mut vector: Vec<_> = insert.iter().map(|x| Some(*x)).collect();
    let remove_sequence = &[3, 1, 4, 5, 4, 3, 0, 1];

    // check that the same swap remove sequence on vec and set
    // have the same result.
    for &rm in remove_sequence {
        let out_vec = vector[rm].take();
        let out_set = set.remove_index(rm);
        assert_eq!(out_vec, out_set);
    }

    let non_dup_len = remove_sequence.iter().collect::<HashSet<_>>().len();
    assert_eq!(vector.len() - non_dup_len, set.len());
    for (i, b) in vector.iter().enumerate() {
        let a = set.get_index(i);
...
fn partial_eq_and_eq() {
    ...
    let set_c: HashSlabSet<_> = set_b.into_iter().collect();
...
#[test]
fn comparisons() {
    let set_a: HashSlabSet<_> = (0..3).collect();
    let set_b: HashSlabSet<_> = (3..6).collect();
    let set_c: HashSlabSet<_> = (0..6).collect();
    let set_d: HashSlabSet<_> = (3..9).collect();
...
fn iter_comparisons() {
    ...
    where
        I1: Iterator<Item = &'a i32> + fmt::Debug,
        I2: Iterator<Item = i32> + fmt::Debug,
    {
        let mut vec1 = Vec::from_iter(iter1.cloned());
        vec1.sort();
        let mut vec2 = Vec::from_iter(iter2);
        vec2.sort();
        assert_eq!(vec1, vec2);
    }
    ...
    let set_b: HashSlabSet<_> = (3..6).collect();
    let set_c: HashSlabSet<_> = (0..6).collect();
    let set_d: HashSlabSet<_> = (3..9).rev().collect();
